...
function parseTelegramStats(channel, html) {
  ...
  const growth30dPct = null;
  const growth30dVal = null;
...
async function generatePayload(input) {
  ...
  // If we have real historical data from MaxChart, compute the actual 30-day growth
  if (max.exists && max.stats?.openData?.subscribersTrendPoints?.length >= 2) {
    const trend = max.stats.openData.subscribersTrendPoints;
    const oldest = trend[0];
    const newest = trend[trend.length - 1]; // or max.stats.subscribers

    if (oldest > 0) {
      const growthVal = newest - oldest;
      const growthPct = round((growthVal / oldest) * 100, 2);

      telegram.growth30d = growthPct;
      telegram.growth30dValue = growthVal;
      max.stats.growth30d = growthPct;
      max.stats.growth30dValue = growthVal;
    }
  }
